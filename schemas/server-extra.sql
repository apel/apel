...
-- TODO Remove factors of 1000 depending on whether SI2K or kSI2K is used
...
CREATE VIEW VSumCPU AS
    SELECT
        ...
        ROUND(SUM((CpuDuration * IF((ServiceLevelType = 'HEPSPEC'),(ServiceLevel * 250),ServiceLevel)) / (1000 * 3600)), 0) AS NormSumCPU,
        ROUND((SUM(WallDuration) / 3600), 0) AS SumWCT,
        ROUND(SUM((WallDuration * IF((ServiceLevelType = 'HEPSPEC'),(ServiceLevel * 250),ServiceLevel)) / (1000 * 3600)), 0) AS NormSumWCT,
        Month,
        Year,
...
DROP VIEW IF EXISTS HepSpecHistory;
