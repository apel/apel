...
def make_schema_test(schema):
    ...
    def schema_test(self):
        if schema == 'server-extra.sql':
            ...
            parent_schema_path = os.path.join(
            ...
            call(
                ...
                stdin=parent_schema_handle
            )
            parent_schema_handle.close()
        schema_path = os.path.join(os.path.dirname(__file__), '..', 'schemas', schema)
...
for schema in os.listdir(os.path.join(os.path.dirname(__file__), '..', 'schemas')):
